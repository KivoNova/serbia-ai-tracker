def classify_news(text_en):
    ...
    if any(word in text_lower for word in ['tech', 'software', 'it ', 'oracle', 'google', 'digital', 'ai', 'computer', 'app', 'code', 'microsoft']):
        return 'Tech'
    elif any(word in text_lower for word in ['energy', 'fuel', 'oil', 'gas', 'price', 'barrel', 'electricity', 'power']):
        return 'Energy'
    elif any(word in text_lower for word in ['bank', 'inflation', 'finance', 'interest', 'nbs', 'money', 'economy', 'tax', 'debt', 'salary', 'gdp']):
        return 'Finance'
    else:
        return 'All'
